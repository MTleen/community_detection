...
if __name__ == "__main__":
    ...
    parser.add_argument('--label_num', default='12',
                        help='节点标签数量')
    parser.add_argument('--idims', default=128, type=int,
                        help='原始嵌入特征维度')
    ...
    G = nx.read_gml(args.label)
    nodes = G.nodes()
    node_values = G._node
    # labels = data[args.label_format].toarray()
    labels = []
    labels_one_hot = np.zeros([len(nodes), args.idims], dtype=np.int)
    ...
    for i in range(16):
        ...
        ax1.scatter(Y[:, 0], Y[:, 1], c=labels)
        # pylab.show()
    fig.savefig(''.join([args.result, '.png']))
