...
from expriments.community_detection.utils.serialization import load_checkpoint
from expriments.community_detection.utils.utils import bcubed
from expriments.community_detection.utils.graph import graph_propagation, graph_propagation_soft, graph_propagation_naive
...
if __name__ == '__main__':
    ...
    # Test args
    parser.add_argument('--checkpoint', type=str, metavar='PATH', default='./logs/adam_epoch_50.ckpt')
    args = parser.parse_args()
    args.val_feat_path = osp.join(working_dir, 'embedding\\%s_embedding.npy' % args.dataset)
    args.val_knn_graph_path = osp.join(working_dir, 'embedding\\%s_knn_graph.npy' % args.dataset)
    args.val_label_path = osp.join(working_dir, 'embedding\\%s_labels.npy' % args.dataset)
    main(args)
